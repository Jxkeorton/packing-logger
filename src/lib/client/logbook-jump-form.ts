...
// The full entry list, kept in memory so entering edit mode can look a
// jump's fields up by id instead of round-tripping every field through
// data-* attributes on its list row (an earlier version of this did that
// — ten attributes per `<li>`, in both the server-rendered markup and the
// logbookRowHtml() below, that had to be kept in step). Seeded once from
// the JSON island LogbookEntryList.astro renders, then kept current by
// applyLogbookState.
let entries: LogbookEntryData[] = [];

function readInitialEntries(): LogbookEntryData[] {
  const el = document.getElementById('logbookEntriesData');
  if (!el?.textContent) return [];
  try {
    return JSON.parse(el.textContent) as LogbookEntryData[];
  } catch (err) {
...
function logbookRowHtml(e: LogbookEntryData): string {
  const description = e.description
    ...
    : '';
  return `
    <li class="logbook-entry" data-at="${e.at}">
...
function renderLogbookList() {
  ...
  if (!list) return;
  list.innerHTML =
    entries.length === 0 ? '<li class="logbook-empty">No jumps logged yet.</li>' : entries.map(logbookRowHtml).join('');
}
 ...
 */
export function applyLogbookState(newEntries: LogbookEntryData[], nextNumber: number) {
  entries = newEntries;
  renderLogbookList();
...
export function initLogbookJumpForm() {
  ...
  function enterEditMode(at: string) {
    const entry = entries.find((e) => e.at === at);
    if (!logbookForm || !entry) return;
    logbookForm.dataset.editing = 'true';
    if (logbookEditingAt) logbookEditingAt.value = entry.at;
    if (logbookDate) logbookDate.value = entry.date;
    if (logbookAltitude) logbookAltitude.value = entry.exitAltitude;
    if (logbookDescription) logbookDescription.value = entry.description;
    selectOptionByText(logbookPlace, entry.place);
    selectEquipmentMatching(entry.canopy, entry.container, entry.aad);
    selectOptionByText(logbookAircraft, entry.aircraft);
    selectOptionByText(logbookJumpType, entry.jumpType);
    ...
    if (logbookNextNumberEl) logbookNextNumberEl.textContent = `#${entry.number}`;
  ...
  logbookList?.addEventListener('click', async (event) => {
    ...
    if (editButton?.dataset.edit) {
      enterEditMode(editButton.dataset.edit);
      return;
    }
