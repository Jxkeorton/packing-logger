...
export interface InvoicePdfOptions {
  ...
  /**
   * Miscellaneous entries billing into this invoice period (from
   * misc-entries.ts's entriesInRange) — the generic sibling of
   * groundSchoolEntries above: each carries its own label and amount
   * rather than fitting a Category or a fixed session name. Renders as
   * its own section, only when non-empty, right after ground school.
   */
...
export function buildSummaryLine(counts: {
  ...
  // AFF, ground school, misc and each handy-cam kind only earn a mention
  // when there were some — an invoice month with none of them reads
  // exactly as it always did, rather than growing a permanent "and 0 AFF
  // instructing jump(s)". Built as a list and joined, rather than each
  // clause carrying its own trailing comma, so two extras in a row don't
  // run into each other ("…(package),, plus…") — only ever one comma,
  // right before "this period", and only when there's at least one extra
  // to introduce it.
  const extras: string[] = [];
  ...
  if (counts.misc > 0) extras.push(`${counts.misc} miscellaneous item(s)`);
  return (
...
export async function buildTandemInvoicePdf(opts: InvoicePdfOptions): Promise<Buffer> {
  ...
  // ---- Miscellaneous ----
  // The generic sibling of the ground school section above: each entry
  // carries its own label (rather than a fixed name like "Ground school
  // session") alongside its own amount, since this ledger covers whatever
  // doesn't fit a Category — "B licence evening" and anything after it —
  // without needing its own section coded here.
  function renderMiscSection(entries: MiscEntry[]): number {
    if (entries.length === 0) return 0;
    if (y > doc.page.height - MARGIN - 100) {
      doc.addPage();
      y = MARGIN;
    ...
    doc.font('Body-Bold').fontSize(10.5).fillColor(NAVY);
    doc.text('Miscellaneous', MARGIN, y, { lineBreak: false });
    y += 16;
    ...
    for (const entry of entries) {
      ...
      doc.text(formatJumpDate(entry.date), MARGIN, y, { width: dateColW, lineBreak: false });
      doc.text(entry.label, MARGIN + dateColW, y, { width: nameColW - 8, lineBreak: false });
      doc.text(money(entry.amount), MARGIN + dateColW + nameColW, y, { width: amountColW, align: 'right', lineBreak: false });
      y += 14;
      subtotal += entry.amount;
    }
    ...
    y += 2;
    doc.font('Body-Bold');
    doc.text(`${entries.length} item(s)`, MARGIN, y, {
      width: pageWidth - amountColW,
      align: 'right',
      lineBreak: false,
    });
  ...
  total += renderMiscSection(opts.miscEntries);
